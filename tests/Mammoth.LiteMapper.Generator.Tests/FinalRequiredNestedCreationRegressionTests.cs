...
{
    [TestClass]
    public sealed class FinalRequiredNestedCreationRegressionTests
    {
        // SPECIFICATION 9.4, 12.4, 14.3, 16.2, and 16.3 require the null check before nested construction.
        [TestMethod]
        public void NullableDestinationCreationChecksRequiredNestedPathBeforeInvokingHelper()
        {
...
{
    public required ChildTarget Child { get; set; }
}
...
public static class Probe
{
    public static bool Run()
    {
        ...
        var present = Mapper.Apply(source, null);
...
");
            ...
            Assert.AreEqual(true, Execute(result.Compilation),
                "Replacement construction must reject a null configured path before the nested helper and map a present child.");
        }

        // SPECIFICATION 9.4, 14.3, 16.2, and 16.3 require null preservation during replacement construction.
        [TestMethod]
        public void NullableDestinationCreationPreservesNullForRequiredNullableNestedMember()
        {
            var result = RunGenerator(@"
#nullable enable
using Mammoth.LiteMapper;
...
public static class Probe
{
    public static bool Run()
    {
        var missing = Mapper.Apply(new Source(), null);

        var source = new Source();
        source.Data.Child = new ChildSource { Value = 3 };
        var present = Mapper.Apply(source, null);

        return missing.Child is null && present.Child?.Value == 3;
    }
}
");
            ...
            Assert.AreEqual(0, result.RunResult.Diagnostics.Length,
                string.Join(Environment.NewLine, result.RunResult.Diagnostics.Select(static diagnostic => diagnostic.ToString())));
            Assert.AreEqual(true, Execute(result.Compilation),
                "Replacement construction must initialize a required nullable nested member to null and map a present child.");
        }
        ...
        private static GeneratorRun RunGenerator(string source)
        {
            ...
            var compilation = CSharpCompilation.Create(
                "FinalRequiredNestedCreationRegressionTests",
