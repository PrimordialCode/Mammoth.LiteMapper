...
namespace Mammoth.LiteMapper.Generator
{
    ...
    public sealed class LiteMapperGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(sortedMappers.Combine(language).Combine(generatorOptions), static (production, input) =>
            {
                var mapperModels = input.Left.Left;
                var version = input.Left.Right;
                ...
                foreach (var mapper in mapperModels)
                {
                    try
                    {
                        ...
                        {
                            production.AddSource(mapper.HintName, SourceText.From(ApplyGeneratorOptions(mapper, version, options), Encoding.UTF8));
                        }
                    }
                    catch (Exception)
                    {
                        if (options.TreatInternalGeneratorErrorsAsExceptions)
                        {
                            throw;
                        }
        ...
        private static MapperModel? CreateMapperModel(GeneratorSyntaxContext context)
        {
            ...
            foreach (var member in symbol.GetMembers().OfType<IMethodSymbol>().Where(static m => m.PartialDefinitionPart == null && IsPartialDeclaration(m)).OrderBy(static m => m.Name, StringComparer.Ordinal))
            {
                ...
                {
                    mappings.Add(CreateUpdateMappingModel(member, context.SemanticModel.Compilation, diagnostics));
                }
            }

            var completedMappings = mappings.ToImmutable();
            foreach (var mapping in completedMappings)
            {
                ...
                {
                    diagnostics.Add(Diagnostic.Create(Diagnostics.RecursiveMappingWithoutCycleDetection, mapping.Method.Locations.FirstOrDefault(), mapping.Method.Name));
                }
                else if (recursiveNames.Count != 0 && mapping.ReferenceHandling == ReferenceHandlingThrowOnCycle &&
                    FlattenHelpers(mapping).Any(h => h.HelperName != null && recursiveNames.Contains(h.HelperName) && h.HelperSourceType != null && h.HelperSourceType.IsValueType))
                {
                    diagnostics.Add(Diagnostic.Create(Diagnostics.CyclePathCannotBeTracked, mapping.Method.Locations.FirstOrDefault(), mapping.Method.Name));
                }
            }
        ...
        private static MappingModel CreateUpdateMappingModel(IMethodSymbol method, Compilation compilation, List<Diagnostic> diagnostics)
        {
            ...
            if (GetCollectionShape(sourceType, compilation)?.Kind == CollectionKind.Array && GetCollectionShape(targetType, compilation)?.Kind == CollectionKind.Array)
            {
            ...
            foreach (var targetMember in GetVisibleMembers(targetType, diagnostics, static m => m is IFieldSymbol field && !field.IsConst || m is IPropertySymbol property && property.Parameters.Length == 0))
            {
                ...
                if (!CanAssignAfterConstruction(targetMember))
                {
                    ...
                    diagnostics.Add(Diagnostic.Create(IsCollectionType(memberType, compilation) || memberType is INamedTypeSymbol namedMemberType && IsStructuralObjectType(namedMemberType) ? Diagnostics.NestedUpdateMappingRequired : Diagnostics.InitOnlyMemberCannotBeUpdated, targetMember.Locations.FirstOrDefault() ?? location, targetMember.Name));
                    continue;
                }
                ...
                var guard = options.IgnoreNullSourceMembers ? BuildPatchGuard(source.Name, explicitConfiguration, match.Member) : null;
                assignments.Add(new AssignmentModel(targetMember.Name, conversion.Expression, guard));
            }
        ...
        private static ConversionModel? ResolveConversion(IMethodSymbol mappingMethod, ExplicitMemberConfiguration? explicitConfiguration, ISymbol? sourceMember, ISymbol targetMember, Compilation compilation, INamedTypeSymbol[] externalTypes, ICollection<Diagnostic> diagnostics, ImmutableArray<MappingModel>.Builder helpers, HashSet<string> helperNames, EffectiveMappingOptions options)
        {
            ...
            var visibleMapping = ResolveVisibleMapping(mappingMethod, mappingMethod.ContainingType, sourceType, targetType, expression, sourceMember, compilation, diagnostics, targetMember.Name, location);
        ...
        private static ConversionModel? ResolveVisibleMapping(IMethodSymbol currentMethod, INamedTypeSymbol mapperType, ITypeSymbol sourceType, ITypeSymbol targetType, string expression, ISymbol? sourceMember, Compilation compilation, ICollection<Diagnostic> diagnostics, string targetName, Location? location)
        {
            var candidates = mapperType.GetMembers().OfType<IMethodSymbol>()
                .Where(m => !SymbolEqualityComparer.Default.Equals(m, currentMethod) && !m.IsImplicitlyDeclared && !HasAttribute(m, MappingConverterAttributeName) && IsUsableConverter(m, mapperType, sourceType, targetType, compilation))
        ...
        private static string RenderEmptyCollection(CollectionShape shape, ITypeSymbol targetType, Compilation compilation)
        {
            ...
            {
                var elementType = shape.ElementType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat).TrimEnd('?');
        ...
        private static string? BuildPatchGuard(string parameterName, ExplicitMemberConfiguration? explicitConfiguration, ISymbol sourceMember)
        {
            ...
            {
                return sourcePath.NullCheckExpressions.Length == 0
                    ? parameterName + "." + sourcePath.Expression + " != null"
                    : string.Join(" && ", sourcePath.NullCheckExpressions.Select(p => parameterName + "." + p + " != null"));
            }

            return SourceMayBeNull(sourceMember) ? parameterName + "." + sourceMember.Name + " != null" : null;
        }
        ...
        private static void AppendMapping(StringBuilder builder, MappingModel mapping, HashSet<string> recursiveHelperNames)
        {
            ...
            else if (trackedHelper)
            {
                builder.Append(", __LiteMapperCycleTracker __tracker, string __memberPath");
            }
            ...
            {
                builder.Append(RewriteTrackedCalls(mapping.CustomBody, recursiveHelperNames, "__memberPath"));
                if (trackedHelper && mapping.HelperSourceType != null && !mapping.HelperSourceType.IsValueType)
                {
                    builder.AppendLine("        }");
            ...
            if (mapping.IsUpdate && destination != null)
            {
                ...
                {
                    if (assignment.Guard != null)
                    {
                        builder.Append("        if (");
                        builder.Append(assignment.Guard);
                        builder.AppendLine(")");
                        builder.AppendLine("        {");
                        builder.Append("            ");
                    }
                    ...
                    builder.Append(" = ");
                    builder.Append(RewriteTrackedCalls(assignment.Expression, recursiveHelperNames, PathExpression(mapping, assignment.TargetName)));
                    builder.AppendLine(";");
                    if (assignment.Guard != null)
                    {
                        builder.AppendLine("        }");
        ...
        private static string PathExpression(MappingModel mapping, string memberName)
        {
            return mapping.HelperName == null ? "\"" + memberName + "\"" : "__memberPath + \"." + memberName + "\"";
        }

        private static string RewriteTrackedCalls(string expression, HashSet<string> recursiveHelperNames, string pathExpression)
        {
            foreach (var helperName in recursiveHelperNames.OrderByDescending(static n => n.Length))
            {
                expression = RewriteTrackedCall(expression, helperName, pathExpression);
            }

            return expression;
        }

        private static string RewriteTrackedCall(string expression, string helperName, string pathExpression)
        {
            ...
            while (index >= 0)
            {
                ...
                if (depth == 0)
                {
                    expression = expression.Insert(position - 1, ", __tracker, " + pathExpression);
        ...
        {
            public AssignmentModel(string targetName, string expression, string? guard = null)
            {
