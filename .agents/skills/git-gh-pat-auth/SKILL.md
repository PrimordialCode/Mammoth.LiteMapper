...
description: "Use when: authenticating git and GitHub CLI for Mammoth.LiteMapper tasks, fixing gh auth errors, setting PAT environment variables, preparing a shell session for git push and gh issue or pr commands."
...
## Outcome
Prepare the current shell session so both git remote operations and gh CLI commands authenticate using a personal access token from the environment variable GITHUB_PrimordialCode.
...
## Required Input
- Environment variable GITHUB_PrimordialCode containing a valid GitHub PAT.
...
2. If missing, stop and request the user to set GITHUB_PrimordialCode securely.
3. Export GH_TOKEN from GITHUB_PrimordialCode for gh CLI in the current session.
...
- GH_TOKEN is populated from GITHUB_PrimordialCode in the active shell.
...
```powershell
$env:GH_TOKEN = $env:GITHUB_PrimordialCode
if ([string]::IsNullOrWhiteSpace($env:GH_TOKEN)) { throw "GITHUB_PrimordialCode is not set" }
gh auth status
```
...
```bash
export GH_TOKEN="$GITHUB_PrimordialCode"
if [ -z "$GH_TOKEN" ]; then echo "GITHUB_PrimordialCode is not set"; exit 1; fi
gh auth status
```
...
```
gh api repos/PrimordialCode/Mammoth.LiteMapper > /dev/null
```
