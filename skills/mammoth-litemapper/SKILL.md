...
For simple matching, public instance properties and fields map by name. The default is exact matching followed by a unique case-insensitive match. Ordinary unmapped targets are errors; unmapped sources are ignored. Explicit `UnmappedTargetMembers` values (`Ignore`, `Info`, `Warning`, or `Error`) override that default. Reported unmapped-member diagnostics identify the exact source or target member. `IgnoreTarget` and `UseTargetDefault` are explicit member-level opt-outs. Required or otherwise mandatory targets still cause errors.
