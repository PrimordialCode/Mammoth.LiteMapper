...
- Workflow handoff: `.github/workflows/ci.yml` performs one pack/validate/upload phase, a checksum/provenance verification phase, and an approval-gated publish phase. Dry runs execute the validation and artifact handoff but never invoke NuGet push. `publish-nuget.ps1` enforces the same exact six-artifact and no-duplicate-suppression contract for local releases.
...
- Current evidence (2026-09-17): ReleaseQualityContractTests 14/14; solution build 0 warnings/errors; Milestone 14 24 passed with one expected Windows Native AOT prerequisite skip; solution 649 passed with the same one expected skip; all six local packages packed successfully and passed package-content checks; Basic, Collections, and ASP.NET Core smoke samples passed sequentially. `dotnet tool restore`, package-level `apicompat`, and the BenchmarkDotNet generated-project smoke were blocked by local NuGet SSL access (zero benchmark cases executed); remote Actions has not yet run.
...
- Status: Option A was approved by the user and incorporated into `SPECIFICATION.md`; red-first regression coverage, minimal implementation, focused validation, integrated local validation, review, merge, and issue closure are complete through PR #9.

## GitHub issue #4 unmapped-diagnostic suppression checkpoint (2026-09-16)

- Objective: synchronize documentation for the safety boundary around the default ordinary unmapped-target diagnostic.
