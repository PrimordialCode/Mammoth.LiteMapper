...
- Status: Implemented on 2026-06-18 with explicit analyzer packing in the primary and generator packages, local-feed consumer package tests, independent abstractions package consumer validation, deterministic package payload checks, trimmed package-consumer publish/run validation, Basic sample runtime coverage for static mapping, instance mapping, nested collections, and cycle detection, and package API compatibility validation.
- Deviations: Native AOT publish validation is present in the packaging test suite but was inconclusive locally because the Windows platform linker prerequisite is not installed. The source-tree Basic sample remains project-reference based; package-consumer publish tests are the authoritative trimming/AOT validation because project-reference publish propagates publish properties into netstandard library projects.
- Remaining tasks: run Native AOT publish validation on a machine or CI runner with platform linker prerequisites installed.
- Validation commands: `dotnet test tests\Mammoth.LiteMapper.Packaging.Tests\Mammoth.LiteMapper.Packaging.Tests.csproj --no-restore --filter Milestone14PackagingAndAotTests` initially failed for missing analyzer assets and package generator activation, then passed with 6 tests and 1 inconclusive Native AOT prerequisite result; `dotnet restore Mammoth.LiteMapper.sln` passed; `dotnet run --project samples\Mammoth.LiteMapper.Samples.Basic\Mammoth.LiteMapper.Samples.Basic.csproj --no-restore` passed; `dotnet build Mammoth.LiteMapper.sln --no-restore` passed with 0 warnings; `dotnet test Mammoth.LiteMapper.sln --no-build` passed with 83 tests and 1 inconclusive Native AOT prerequisite result; `dotnet pack src\Mammoth.LiteMapper.Abstractions\Mammoth.LiteMapper.Abstractions.csproj -c Release -o artifacts\packages` passed with NuGet readme warning; `dotnet pack src\Mammoth.LiteMapper.Generator\Mammoth.LiteMapper.Generator.csproj -c Release -o artifacts\packages` passed with NuGet readme warning; `dotnet pack src\Mammoth.LiteMapper\Mammoth.LiteMapper.csproj -c Release -o artifacts\packages` passed with NuGet readme warning; `apicompat package artifacts\packages\Mammoth.LiteMapper.Abstractions.1.0.0.nupkg --run-api-compat` passed; `apicompat package artifacts\packages\Mammoth.LiteMapper.1.0.0.nupkg --run-api-compat` passed; `git diff --check` passed.
- Risks: AOT support depends on SDK workloads/platform; NuGet pack emits non-fatal missing-readme warnings for all three packages.
