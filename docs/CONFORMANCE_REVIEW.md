...
### GitHub issue #7 strict source-member completeness (2026-09-16)

The existing `UnmappedSourceMembers` option is now presented as an explicit opt-in boundary-mapping guardrail. The usage guide and consumer skill explain mapper, method, and assembly configuration, `LITEMAPPER1003`, staged `Warning`/`Info` adoption, `IgnoreSource` exceptions, and why the library default remains `Ignore` for source models with intentionally broader data. Generator-only regression coverage proves strict `Error` behavior at all three scopes; existing severity rows cover `Error`, `Warning`, `Info`, `Ignore`, and editorconfig overrides without requiring consumer DTO tests. Target completeness and mandatory diagnostics remain unchanged. Focused generator coverage passes 27/27, the full generator suite passes 564/564, the solution build passes with 0 warnings/errors, solution tests pass 629 with 1 expected Windows Native AOT linker-prerequisite skip, the usage documentation test passes 1/1, the consumer skill validator passes, and the CRLF-aware diff check passes. PR #10 was approved and merged into `develop` at `a8ac8bd`; issue #7 is closed and remote CI remains unexecuted.
