...
The existing `UnmappedSourceMembers` option is now presented as an explicit opt-in boundary-mapping guardrail. The usage guide and consumer skill explain mapper, method, and assembly configuration, `LITEMAPPER1003`, staged `Warning`/`Info` adoption, `IgnoreSource` exceptions, and why the library default remains `Ignore` for source models with intentionally broader data. Generator-only regression coverage proves strict `Error` behavior at all three scopes; existing severity rows cover `Error`, `Warning`, `Info`, `Ignore`, and editorconfig overrides without requiring consumer DTO tests. Target completeness and mandatory diagnostics remain unchanged. Focused generator coverage passes 27/27, the usage documentation test passes 1/1, the consumer skill validator passes, and the CRLF-aware diff check passes. Full validation and remote CI remain pending.
