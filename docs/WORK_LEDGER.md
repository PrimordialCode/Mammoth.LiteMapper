...
This is a documentation synchronization and focused regression unit on `codex/issue-4-unmapped-diagnostic-suppression`. The synchronized contract keeps ordinary unmapped target members at `Error` by default and keeps `LITEMAPPER1001` configurable for explicit policy and standard compiler severity configuration. Ordinary suppression or downgrade does not suppress generation and is not proof of complete mapping. Required, non-nullable, inaccessible, invalid, and otherwise mandatory target obligations remain hard non-configurable errors; `IgnoreTarget` and `UseTargetDefault` retain existing validation. CI guidance recommends treating `LITEMAPPER1001` as an error and auditing or avoiding project-wide suppression when mandatory coverage is required. The focused `UnmappedDiagnosticSeverityTests` regression proves that suppressing the default ordinary diagnostic retains valid generated code. No production-source, public API, diagnostic, or semantic behavior changes are in scope. Scoped contradiction search and diff checks pass; skill validation passes; the focused usage-documentation test passes 1/1. Committed as `5ea413f`, merged into `develop`, pushed, and issue #4 closed.
